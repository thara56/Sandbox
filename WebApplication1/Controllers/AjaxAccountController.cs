...
namespace WebApplication1.Controllers
{
    ...
    {
        public AjaxAccountController() : this(IdentityConfig.Secrets, IdentityConfig.Logins, IdentityConfig.Users, IdentityConfig.Roles, IdentityConfig.Bearer, IdentityConfig.ExternalIdentityHandler) { }

        public AjaxAccountController(IUserSecretStore secrets, IUserLoginStore logins, IUserStore users, IRoleStore roles, OAuthBearerAuthenticationOptions bearer, ISecureDataHandler<ClaimsIdentity> externalIdentityHandler)
        {
            Secrets = secrets;
            Logins = logins;
            Users = users;
        ...
        [HttpGet("api/AjaxAccount/UserInfo")]
        ...
        [AllowAnonymous]
        [HttpPost("api/AjaxAccount/Login")]
        public async Task<object> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                if (await Secrets.Validate(model.UserName, model.Password))
                {
                    ...
                    IEnumerable<Claim> claims = await FindClaims(userId, new Claim[0]);
                    ClaimsIdentity cookieIdentity = CreateCookieIdentity(claims);
                    Grant(cookieIdentity, model.RememberMe);
        ...
        [AllowAnonymous]
        [HttpGet("api/AjaxAccount/ExternalLogin", RouteName = "ExternalLogin")]
